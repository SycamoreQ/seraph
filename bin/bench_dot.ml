(* bin/bench_dot.ml
   hyperfine-drivable benchmark binary for dot-product variants.

   Usage:
     bench_dot <variant> <n> [reps]

   variant ∈ { scalar | neon | optim | clang | clang_restrict }
*)
...
type fn = B.f32arr -> B.f32arr -> int -> float

let variant_fn : string -> fn = function
  | "scalar" -> fun x y _n -> S.dot_prod x y
  | "neon" -> B.dot_neon
  | "optim" -> B.dot_optim
  | "clang" -> B.dot_prod_gen
  | "clang_restrict" -> B.dot_prod_restrict_gen
  | other ->
    Printf.eprintf
...
let () =
  ...
  (* Prevent the whole loop from being dead-code eliminated. *)
